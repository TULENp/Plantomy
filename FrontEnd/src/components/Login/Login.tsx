...
export function Login({active, setActive}): JSX.Element{
    return(
        <div className={active ? "cont_login active" : "cont_login"} onClick={()=> {setActive(false)}}>
            ...
            <img className='btn_exit' src='src\Assets\exit.svg' onClick={()=> {setActive(false)}}></img>
                ...
                <Button type='primary' className='btn_login_login'>Войти</Button>
                <Button type='ghost' className='btn_reg'>Регистрация</Button>
            </div>
        </div>
    )
}